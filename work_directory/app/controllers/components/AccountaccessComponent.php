...
class AccountaccessComponent extends Component
{
    ...
    public function manageUsers()
    {
        ...
        $possible_arguments = ["adduser", 'edituser', 'search',
            "deleteuser", "deleteusers", "adduserrole", "deleteuserrole",
        ...
        if ($username == "" && isset($_REQUEST['arg']) &&
            $_REQUEST['arg'] != "search" && $_REQUEST['arg'] != "deleteusers") {
            unset($_REQUEST['arg']);
        }
        ...
        if (isset($_REQUEST['arg']) &&
            ...
            switch ($arg) {
                ...
                case "deleteuser":
                    ...
                    break;
                case "deleteusers":
                    if (!isset($_REQUEST['user_names'])) {
                        return;
                    }

                    $usernames = explode(',', $_REQUEST['user_names']);
                    foreach ($usernames as $username) {
                        $user_id = $signin_model->getUserId($username);
                        if ($user_id <= 0 || in_array(
                                $user_id, [C\ROOT_ID, C\PUBLIC_USER_ID])) {
                            continue;
                        } else {
                            $user_model->deleteUser($username);
