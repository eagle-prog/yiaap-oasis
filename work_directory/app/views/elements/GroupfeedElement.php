...
class GroupfeedElement extends Element implements CrawlConstants
{
        ...
        &$data)
    {
        ...
        foreach ($data['PAGES'] as $page) {
            ...
            if (!$is_group) {
                ...
                $content = $description . ($edit_date ? "(<b>". tl('groupfeed_element_edited', $edit_date). "</b>)" : ""); ?>
                <div id='description<?= $page['ID']?>'>
    ...
    private function drawStartThreadForm($group_id, $data)
    {
        ...
        <div id='start-thread' class="light-gray-box top-bottom-margin">
            ...
            <div id="description-start-thread"></div>
            <div>
        ...
        {
            $('#description-start-thread').summernote({
                placeholder: '',
                tabsize: 2,
                height: 180,
                toolbar: [
            ...
            $('#start-thread form').submit(function() {
                const code = $('#description-start-thread').summernote('code');
                $('#start-thread form input[name="description"]').val(code);
            });
    ...
    private function drawCommentForm($thread_id, $group_id, $data)
    {
        ...
        <input type="hidden" name="<?= C\CSRF_TOKEN ?>"
            value="<?= $data[C\CSRF_TOKEN] ?>" />
        <input type="hidden" name="description" value="">
        <div id="comment-add-comment"></div>
        ...
        {
            $('#comment-add-comment').summernote({
                placeholder: '',
                tabsize: 2,
                height: 180,
                toolbar: [
            ...
            $('#add-comment form').submit(function() {
                const code = $('#comment-add-comment').summernote('code');
                $('#add-comment form input[name="description"]').val(code);
            });
    ...
    public function renderScripts($data, $with_status_update = false)
    {
        ...
        function commentForm(id, parent_id, group_id)
        {
            ...
            if (start_elt != tmp) {
                ...
                $(`#${id} form`).submit(function() {
                    const code = $(`#${comment_id}`).summernote('code');
                    $(`#${id} form input[name="description"]`).val(code);
                });
        ...
        function updatePostForm(id)
        {
            ...
            let description = elt('old-description'+id).innerText;
            ...
            if (start_elt != tmp) {
                ...
                $(`#${description_id}`).summernote('code', description);
                $(`#${id} form`).submit(function() {
                    const code = $(`#${description_id}`).summernote('code');
                    $(`#${id} form input[name="description"]`).val(code);
                });
            } else {
                elt(id).innerHTML = "";
