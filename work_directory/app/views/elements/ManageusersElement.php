...
class ManageusersElement extends Element
{
    ...
    public function renderUserTable($data)
    {
        ...
        $num_columns = $_SERVER["MOBILE"] ? 4 : 9;
        ...
        <table class="admin-table">
            ...
            <tr>
                <th><input type="checkbox" class="chk-select-all"></th>
            ...
            foreach ($data['USERS'] as $user) { ?>
                <tr>
                <td><input type="checkbox" class="chk-select" data-user-name="<?= $user['USER_NAME'] ?>"></td><?php
    ...
    public function renderUserForm($data)
    {
        ...
        function updateDeleteButtonVisible() {
            <?php
                $base_url = htmlentities(B\controllerUrl('admin', true)) .
                C\CSRF_TOKEN . "=" . $data[C\CSRF_TOKEN] . "&a=manageUsers";
            ?>

            var checkedChks = document.querySelectorAll('.chk-select:checked');
            var btnDeleteSelected = document.querySelector('.delete-selected-rows');
            var deleteUrl = '<?= $base_url . "&arg=deleteusers&user_names="; ?>';

            if (!btnDeleteSelected) { return; }
            if (checkedChks.length > 0) {
                for (const chk of checkedChks) {
                    deleteUrl += chk.getAttribute('data-user-name') + ',';
                }

                btnDeleteSelected.style.display = 'inline-block';
                btnDeleteSelected.href = deleteUrl;
            } else {
        ...
        window.onload = function() {
            var chkSelectAll = document.querySelector('.chk-select-all');
            var chks = document.querySelectorAll('.chk-select');
            ...
            chks.forEach(function(chk) {
                chk.addEventListener('change', updateDeleteButtonVisible);
            });
        }
        </script>
        <?php
