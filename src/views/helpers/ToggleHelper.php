    ...
    public function render($state, $on_url, $off_url, $caution = false)
    {
        ...
        if ($state) {
            ...
            if ($caution) { ?>
                ...
                class="<?= $oncolor
                    ?>"><a <?=$target?> href="<?= $on_url ?>"><?=
                tl('toggle_helper_on') ?></a></td>
                <td><a <?=$target?> href="<?= $off_url ?>"
                ><?= tl('toggle_helper_off') ?></a></td></tr></table>
            <?php } else { ?>
                <table class="toggle-table"><tr><td class="<?= $oncolor
                    ?>"><b><?= tl('toggle_helper_on') ?></b></td>
                <td><a <?=$target?> href="<?= $off_url ?>"
                ><?= tl('toggle_helper_off') ?></a></td></tr></table>
            <?php } ?>
        <?php } else {?>
            <table class="toggle-table"><tr><td><a <?=$target
                ?> href="<?= $on_url ?>" ><?= tl('toggle_helper_on') ?></a></td>
            <td  class="back-red"><b><?=
